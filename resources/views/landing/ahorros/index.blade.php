...
@section('titulo', 'Ahorros')
@section('contenido')

<div id=portada class="oculto">
@if (isset($portada))
    <div class="wrap-login100-img">
        <img src="{{ asset($portada->elemento->ELEM_ValorAhorro) }}">
    </div>
@endif
</div>
...
@if (isset($portadaMovil))
    <div class="wrap-login100-img">
        <img src="{{ asset($portadaMovil->elemento->ELEM_ValorAhorro) }}">
    </div>
@endif
</div>
            ...
            @switch($dato->elemento->ELEM_Tipo)
                @case('L')
                    <span class="login100-form-title p-b-2 p-t-27">
                        {{ $dato->elemento->ELEM_ValorAhorro }}
                    </span>
                    @break
                    ...
                    <div>
                        ...
                        <input id="{{ $dato->elemento->ELEM_ValorCampo }}" type="checkbox" value="yes" name="{{ $dato->elemento->ELEM_ValorCampo }}">
                            @if($dato->elemento->ELEM_ValorAuxiliar === 'REF')
                                <a href="{{ asset('img/autorizacion.pdf') }}" target="_blank">{{ $dato->elemento->ELEM_ValorAhorro }}</a>
                            @else
                                {{ $dato->elemento->ELEM_ValorAhorro }}
                            @endif
                        </label>
